...
Graph * readDimacsGraph(char *filename) {
    Graph *g = nullptr;
    ifstream f(filename);
...
Graph *readGraph(char *filename, char format) {
    if (format == 'D') return readDimacsGraph(filename);
    else if (format == 'L') return readLadGraph(filename);
    else if (format == 'B') return readBinaryGraph(filename);
