...
struct Graph readDimacsGraph(const char* filename, bool directed, bool vertex_labelled) {
    struct Graph g(0);

    std::ifstream f(filename);

    if (!f)
        fail("Cannot open file: " + std::string(filename));
...
struct Graph readLadGraph(const char* filename, bool directed) {
    struct Graph g(0);
    FILE* f;
...
struct Graph readBinaryGraph(const char* filename, bool directed, bool edge_labelled,
...
struct Graph readGraph(const char* filename, char format, bool directed, bool edge_labelled, bool vertex_labelled) {
    struct Graph g(0);
